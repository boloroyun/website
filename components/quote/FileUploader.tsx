...
export function FileUploader({ onImagesUploaded, onError }: FileUploaderProps) {
  ...
  const getFileIcon = (fileType: string) => {
    ...
    if (fileType === 'application/pdf') return <File className="h-6 w-6" />;
      ...
      {files.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {files.map((fileItem) => (
            <div
              key={fileItem.id}
              className="relative border rounded-md overflow-hidden bg-gray-50 h-36 flex flex-col"
            >
              {/* Preview */}
